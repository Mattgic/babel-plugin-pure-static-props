    ...
    visitor: {
      Program: {
        enter(programPath) {
          ...
          programPath.traverse({
            ClassDeclaration: {
              exit(path) {
                if (isReactClass(path.get('superClass'), path.scope, {})) {
                  ...
                  if (hasStaticProperties) {
                    const componentNameIdentifier = path.node.id;
        ...
        exit() {
          ...
          functionComponentStaticProperties.forEach(
            (staticProperties, functionComponentPath) => {
              const componentNameIdentifier = functionComponentPath.node.id;
              ...
              const functionNode = functionComponentPath.isVariableDeclarator()
                ? t.VariableDeclaration('const', [functionComponentPath.node])
      ...
      AssignmentExpression(path, state) {
        ...
        if (isStatelessComponent(binding.path)) {
          ...
          functionComponentStaticProperties.set(funcPath, staticProperties);
